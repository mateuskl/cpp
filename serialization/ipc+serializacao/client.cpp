...
void MyFrame::OnRequest(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        //
        std::string nome = "Mateus Krepsky Ludwich";
        char sexo = 'm';
        int idade = 22;
        std::ostringstream representacao_serial;
        Pessoa::Ptr mateus (new Pessoa(nome, sexo, idade));
        //
        char* data;
        int size;        
        ...
        data = (char*) m_client->GetConnection()->Request(_T("pessoa"), &size);        
        //        
        assert(data);
        assert(size == 30);        
        std::string rep_serial = data;
        assert(rep_serial == "Mateus Krepsky Ludwich $ m 22");
        Pessoa::Ptr mateusReconstruido = Pessoa::unserialize(rep_serial);
        assert (mateus->ehIgual(mateusReconstruido));        
    }
}
