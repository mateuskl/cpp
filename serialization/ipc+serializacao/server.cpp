    ...
    const wxString& item, int * size, wxIPCFormat format)
{
    ...
    else if (item == _T("pessoa"))
    {
        ...
        mateus->serialize(representacao_serial);
        std::string rep_serial= representacao_serial.str();    
        data = (wxChar *) rep_serial.c_str();
        *size = (rep_serial.length() + 1) * sizeof(wxChar);        
    }
    else
    {
        data = NULL;
